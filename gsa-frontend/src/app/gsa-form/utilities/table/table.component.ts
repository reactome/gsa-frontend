...
export class TableComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.data$ = this.store.select(selectors.selectData(TableOrder.ROW_BY_ROW));

    this.renameValue = this.settings.columns[0];
    this.start$ = this.store.select(selectors.selectStart);
    this.startCell$ = this.start$.pipe(
      map(start => this.getHTMLCellElement(start.x, start.y)?.getBoundingClientRect()),
      ...
      map(cell => {
        ...
        cell.x += tablePosition?.scrollLeft - tableCoords?.x - 1;
        cell.y += tablePosition?.scrollTop - tableCoords?.y - 1;
        cell.height -= 3;
        cell.width -= 19;
        return cell;
      })
    );
    this.stop$ = this.store.select(selectors.selectStop);
    this.inputVisible$ = this.store.select(selectors.selectHasFocus).pipe(tap(bool => console.log('Has focus ' + bool)));

    this.inputValue$ = this.store.select(selectors.selectValue).pipe(tap(value => console.log("input cell changed", value)));
    ...
    this.store.dispatch(TableActions.import({
      table: [
        ...
        ...this.settings.data.map((row, y) => [this.settings.rows[y], ...row.map(cell => cell.value)])
      ],
      hasColNames: true,
      hasRowNames: true,
  ...
  renameCell() {
    ...
    this.store.dispatch(TableActions.write({value: this.value}));
  ...
  copyValues() {
    ...
    this.store.select(selectors.selectRange)
      .pipe(first())
