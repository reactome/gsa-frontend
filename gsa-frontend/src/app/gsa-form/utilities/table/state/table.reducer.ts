...
import {Coords, initialState, State} from "./table.state";
...
export const reducer: ActionReducer<State> = createUndoRedoReducer(
  ...
  on(TableActions.write, (state, {value}) => {
    state.hasFocus = true;
    state.dataset[state.start.y][state.start.x].value = value;
      ...
      case TableOrder.ROW_BY_ROW:
        for (let yFrom = 0; yFrom < table.length; yFrom++) {
          ...
          if (hasRowNames && nameToRowI.has(rowName)) {
            ...
            yTo = state.dataset.push(state.dataset[0].map(() => ({value: ''}))) - 1;
  ...
  on(TableActions.addColumn, (state) => {
    const x = pushAll(state.dataset, ({value: ''})) - 1;
  ...
  on(TableActions.addRow, (state) => {
    const y = state.dataset.push(state.dataset[0].map(() => ({value: ''}))) - 1;
...
namespace Cells {
  export function select(state: State): State {
    state.selectedCoords.forEach(coords => state.dataset[coords.y][coords.x].selected = false);
