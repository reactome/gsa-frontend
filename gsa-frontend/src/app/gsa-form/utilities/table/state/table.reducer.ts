...
export const reducer: ActionReducer<State> = createUndoRedoReducer(
  ...
  on(TableActions.down, (state, {shift}) => {
    ...
    if (state.start.y !== limits.y.max) {
      ...
      state.start.y = limits.y.min;
      state.start.x = (state.start.x !== limits.x.max) ? state.start.x + 1 : limits.x.min;
    }
  ...
  on(TableActions.up, (state, {shift}) => {
    const limits = Ranges.limits(state);
    if (state.start.y !== limits.y.min) {
      state.start.y--;
    } else {
      state.start.y = limits.y.max;
      state.start.x = (state.start.x !== limits.x.min) ? state.start.x - 1 : limits.x.max;
    }
    if (!shift) state.stop = state.start;
    return Cells.select(state);
  }),
  on(TableActions.left, (state, {shift}) => {
    const limits = Ranges.limits(state);
    if (state.start.x !== limits.x.min) {
      state.start.x--;
    } else {
      state.start.x = limits.x.max;
      state.start.y = (state.start.y !== limits.y.min) ? state.start.y - 1 : limits.y.max;
    }
    if (!shift) state.stop = state.start;
    return Cells.select(state);
  }),
  on(TableActions.right, (state, {shift}) => {
    const limits = Ranges.limits(state);
    if (state.start.x !== limits.x.max) {
      state.start.x++;
    } else {
      state.start.x = limits.x.min;
      state.start.y = (state.start.y !== limits.y.max) ? state.start.y + 1 : limits.y.min;
    }
    if (!shift) state.stop = state.start;
    return Cells.select(state);
  }),
...
export namespace Ranges {
  ...
  export function origin(state: State): Coords {
    return {
  ...
  export function limits(state: State): Range {
  ...
  export function limitRange(range: Range, state: State, limits = Ranges.limits(state)): Range {
    return {
